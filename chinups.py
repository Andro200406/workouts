...
# Variables for counting chin-ups
chin_up_count = 0
chin_up_state = "down"  # Track whether the user is in the "down" or "up" position
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if head_y < head_initial_y - 0.1:  # Head moves up by a certain threshold
                if chin_up_state == "down" and left_elbow_angle < 160 and right_elbow_angle < 160:  # Elbows are flexed
                    chin_up_count += 1
                    chin_up_state = "up"  # State changes to "up"

            # Check if the body is coming down (head lowering)
            if head_y > head_initial_y + 0.1:  # Head moves down by a certain threshold
                chin_up_state = "down"  # State changes to "down" when lowering
            
            # Display Chin-up count on the frame
            cv2.putText(image, f'Chin-ups: {chin_up_count}', (50, 50), 
        ...
        # Display the frame
        cv2.imshow('Chin-up Detection', image)
