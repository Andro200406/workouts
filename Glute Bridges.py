...
# Variables for counting Glute Bridges
count = 0
in_bridge = False
last_bridge_time = 0

# Function to check if the person is in the Glute Bridge position
def is_in_glute_bridge(landmarks):
    ...
    # Check if the body forms a straight line from knees to shoulders
    ...
    # Detect Glute Bridge: Check if hips are elevated and knees bent
    ...
    # Check if knees are bent (leg angle less than 45 degrees)
    ...
    is_knee_bent = abs(leg_angle_left) < 0.8 and abs(leg_angle_right) < 0.8  # Adjust knee angle for bent position
    
    return is_hips_elevated and is_body_in_line and is_knee_bent
...
with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            # Check if the person is performing Glute Bridge
            if is_in_glute_bridge(landmarks):
                if not in_bridge:
                    in_bridge = True
                ...
                if duration > 0.5:  # Ensure a significant hold position before counting
                    count += 1  # Count the Glute Bridge movement

            # Display the count on the frame
            cv2.putText(image, f'Glute Bridges: {count}', (50, 50),
