...
# Variables for high plank detection
high_plank_detected = False
plank_time = 0
start_time = None
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if 160 < left_body_angle < 180 and 160 < right_body_angle < 180:
                # Ensure hips are not too high or low compared to shoulders and ankles
                hip_height = left_hip[1] if left_body_angle > right_body_angle else right_hip[1]
                shoulder_height = left_shoulder[1] if left_body_angle > right_body_angle else right_shoulder[1]
                ankle_height = left_ankle[1] if left_body_angle > right_body_angle else right_ankle[1]
                
                if shoulder_height < hip_height < ankle_height:
                    if not high_plank_detected:
                        high_plank_detected = True
            ...
            # Display High Plank hold time
            cv2.putText(image, f'Plank Time: {plank_time:.2f} sec', (50, 50), 
        ...
        # Display the frame
        cv2.imshow('High Plank Detection', image)
