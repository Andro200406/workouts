...
# Variables for counting pull-ups
pull_up_count = 0
pull_up_state = "down"  # Track whether the user is in the "down" or "up" position
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if head_y < head_initial_y - 0.1:  # Head moves up by a certain threshold
                if pull_up_state == "down" and left_elbow_angle < 160 and right_elbow_angle < 160:  # Elbows are flexed
                    pull_up_count += 1
                    pull_up_state = "up"  # State changes to "up"

            # Check if the body is coming down (head lowering)
            if head_y > head_initial_y + 0.1:  # Head moves down by a certain threshold
                pull_up_state = "down"  # State changes to "down" when lowering
            
            # Display Pull-up count on the frame
            cv2.putText(image, f'Pull-ups: {pull_up_count}', (50, 50), 
        ...
        # Display the frame
        cv2.imshow('Pull-up Detection', image)
