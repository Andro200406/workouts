...
# Variables for counting handstand push-ups
push_up_count = 0
...
with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            # Calculate the angle of the body to detect handstand position
            body_angle = calculate_angle(left_shoulder, nose, right_shoulder)

            # Condition to detect if the person is in a handstand position
            if body_angle < 45:  # If the body is nearly vertical (handstand)
            ...
            # Display handstand push-up count on the frame
            cv2.putText(image, f'Handstand Push-ups: {push_up_count}', (50, 50), 
        ...
        # Display the frame
        cv2.imshow('Handstand Push-up Detection', image)
